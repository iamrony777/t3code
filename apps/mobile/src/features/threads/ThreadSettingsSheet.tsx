...
type ThreadSettingsSubmenuPage =
  ...
  | { readonly kind: "global-descriptor"; readonly id: string }
...
export const EMPTY_PROVIDER_GLOBAL_OPTIONS: ReadonlyArray<ProviderGlobalOption> = [];
...
type ThreadSettingsSessionValue = {
  ...
  readonly globalOptions: ReadonlyArray<ProviderGlobalOption>;
  readonly pendingGlobalOptionIds: ReadonlySet<string>;
  readonly applyGlobalOptionChange: (
    descriptor: ProviderGlobalOption,
...
function ThreadSettingsSessionProvider(
  ...
  // Provider-global settings live on the instance, not the staged model, so
  // they apply immediately rather than waiting for Save.
  ...
  const applyGlobalOptionChange = useCallback(
    (descriptor: ProviderGlobalOption, value: string | boolean) => {
      if (!environmentId || !selectedInstanceId) {
        return;
      }
    ...
    [environmentId, props.globalOptions, props.onSetGlobalOption, selectedInstanceId],
  );
  ...
  return (
    <View style={{ paddingBottom: insets.bottom + bottomToolbarInset + 12 }}>
      ...
      >
        ...
        {session.globalOptions.map((descriptor) => {
          const pending = session.pendingGlobalOptionIds.has(descriptor.id);
          return (
            <Animated.View
              key={`global:${descriptor.id}`}
              entering={props.animationsReady ? THREAD_SETTINGS_OPTION_ENTER_TRANSITION : undefined}
              exiting={props.animationsReady ? THREAD_SETTINGS_OPTION_EXIT_TRANSITION : undefined}
              layout={THREAD_SETTINGS_OPTIONS_LAYOUT_TRANSITION}
            >
              {descriptor.type === "select" ? (
                <DisclosureRow
                  ...
                  disabled={pending}
                  onPress={() =>
                    props.onOpenSubmenu({ kind: "global-descriptor", id: descriptor.id })
                  }
                />
              ) : (
                <SwitchRow
                  ...
                  value={descriptor.currentValue ?? false}
                  disabled={pending}
                  onValueChange={(value) => session.applyGlobalOptionChange(descriptor, value)}
                />
              )}
            </Animated.View>
          );
...
function ThreadSettingsChoiceContent(props: {
  ...
  const globalDescriptorId = props.submenu.kind === "global-descriptor" ? props.submenu.id : null;
  ...
  const activeDescriptor =
    descriptorId !== null
      ...
      : undefined;
  const activeGlobalDescriptor =
    globalDescriptorId !== null
      ? session.globalOptions.find(
          (descriptor) => descriptor.type === "select" && descriptor.id === globalDescriptorId,
        )
      : undefined;

  const submenuContent =
    activeGlobalDescriptor?.type === "select"
      ? {
          rows: selectableChoices(activeGlobalDescriptor).map((choice) => ({
            id: choice.id,
            label: choice.label,
            description: undefined,
            selected: choice.id === getProviderOptionCurrentValue(activeGlobalDescriptor),
            onPress: () => {
              void Haptics.selectionAsync();
              session.applyGlobalOptionChange(activeGlobalDescriptor, choice.id);
              props.onSelected();
            },
          })),
        }
