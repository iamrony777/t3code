    ...
    it.effect("uses the project root and stores provider-specific resume cursors", () =>
      Effect.gen(function* () {
        ...
        const directory = ProviderSessionDirectory.ProviderSessionDirectory.of({
          ...
          compareAndSet: () => Effect.die("unused"),
          ...
          getBinding: () =>
            Effect.succeed(
              bindings[0] === undefined
                ? Option.none()
                : Option.some({ ...bindings[0], lastSeenAt: "2026-01-01T00:00:00.000Z" }),
            ),
          listThreadIds: () => Effect.die("unused"),
          listBindings: () => Effect.die("unused"),
        });
    ...
    it.effect("does not replace completed history or an active binding on retry", () =>
      Effect.gen(function* () {
        ...
        const directory = ProviderSessionDirectory.ProviderSessionDirectory.of({
          ...
          getBinding: () =>
            Effect.succeed(
              Option.some({ ...runningBinding, lastSeenAt: "2026-01-01T00:00:00.000Z" }),
            ),
          listThreadIds: () => Effect.die("unused"),
          listBindings: () => Effect.die("unused"),
        });
