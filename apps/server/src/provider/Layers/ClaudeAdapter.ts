...
/**
 * Session system prompt: the preset, plus runtime info and the federated
 * memory block when set. Model and effort can change after this
 * session-level prompt is set, so runtime info omits them.
 */
export function buildClaudeSystemPrompt(memoryContext: string | undefined): {
  ...
  const runtimeInstructions = buildRuntimeInstructions({ harness: "Claude Code" });
  return {
    type: "preset",
    preset: "claude_code",
    append: memoryContext ? `${runtimeInstructions}\n\n${memoryContext}` : runtimeInstructions,
  };
}
