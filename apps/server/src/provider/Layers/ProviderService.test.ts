    ...
    Effect.gen(function* () {
      ...
      Effect.provide(
        MemorySourceIndexer.layerTest({
          ...
          // shapes the injected block for the fixture "project" session cwd.
          providers: { claudeAgent: { enabled: false } },
          memorySources: [
            {
              label: "Claude memory",
              path: MEMORY_FILE,
              projectRoot: fixtureCwd("project"),
              enabled: true,
            },
          ],
        }),
  ...
  it.effect("persists a completed turn cursor before adapter session recovery", () =>
    Effect.gen(function* () {
      ...
      yield* provider.startSession(threadId, {
        ...
        threadId,
        cwd: fixtureCwd("project-command-code"),
        modelSelection,
        runtimeMode: "full-access",
      });
...
const boundedListing = makeProviderServiceLayer({
  directory: {
    upsert: () => Effect.void,
    compareAndSet: () => Effect.die("ProviderService.listSessions does not use compareAndSet"),
