...
const makeProviderService = Effect.fn("makeProviderService")(function* (
  ...
  const persistCommandCodeSessionAfterCompletion = (
    source: {
      readonly instanceId: ProviderInstanceId;
      readonly provider: ProviderDriverKind;
    ...
    Effect.gen(function* () {
      ...
      const session = sessions.find((candidate) => candidate.threadId === canonicalEvent.threadId);
  ...
  const processRuntimeEvent = (
    ...
    Effect.gen(function* () {
      ...
      if (source.provider === "commandcode" && canonicalEvent.type === "turn.completed") {
        yield* persistCommandCodeSessionAfterCompletion(source, canonicalEvent);
      }
      yield* increment(providerRuntimeEventsTotal, {
      ...
      if (canonicalEvent.type === "turn.started") {
        ...
        if (source.provider === "claudeAgent") {
          ...
          yield* Effect.gen(function* () {
            ...
            if (session?.resumeCursor !== undefined && session.status !== "running") {
