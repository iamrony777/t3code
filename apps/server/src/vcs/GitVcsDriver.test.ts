...
it.effect("restores empty checkpoints without changing paths outside the workspace", () =>
  Effect.gen(function* () {
    ...
    for (const nested of [false, true]) {
      ...
      const gitInfo = path.join(root, ".git", "info");
      yield* fileSystem.makeDirectory(gitInfo, { recursive: true });
      yield* fileSystem.writeFileString(path.join(gitInfo, "exclude"), "ignored.txt\n");
