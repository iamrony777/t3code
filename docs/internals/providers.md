...
Command Code runs headlessly, so its interactive permission prompts cannot be relayed. The
[adapter](../../apps/server/src/provider/Layers/CommandCodeProvider.ts) maps runtime modes onto
its fail-closed `dont-ask`, `auto-accept`, and `--yolo` flags, and plan mode onto its read-only
`--plan` flag, instead of surfacing approvals.
